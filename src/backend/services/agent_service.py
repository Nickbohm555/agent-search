...
def run_runtime_agent(payload: RuntimeAgentRunRequest, db: Session) -> RuntimeAgentRunResponse:
    ...
    logger.info("Runtime agent run start query_length=%s", len(payload.query))
    ...
    result = agent.invoke({"messages": [HumanMessage(content=payload.query)]})
    output = _extract_last_message_content(result)
    logger.info("Runtime agent run complete output_length=%s", len(output))
