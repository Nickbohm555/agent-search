...
| Node | How it is done |
| --- | --- |
| `Decompose` | Uses a LangChain chat model with structured output (`DecompositionPlan`) to generate sub-questions, then normalizes/dedupes and caps count. This keeps decomposition deterministic and bounded. |
| `Expand` | Uses the query expansion service (`MultiQueryRetriever.from_llm(...)`) to generate alternate phrasings from each sub-question, then normalizes and limits query count. This increases retrieval recall across wording variants. |
| `Search` | Calls vector similarity retrieval through `search_documents_for_queries(...)` for each expanded query, then merges duplicates by document identity and preserves provenance. This is the main embedding-based recall stage. |
| `Rerank` | Calls the reranker service, which uses an LLM prompt to return a strict JSON relevance ordering and optional scores, then rewrites citation order. This turns high-recall retrieval into high-precision context ordering. |
| `Answer` | Generates one sub-answer from reranked evidence and enforces citation-marker validity (`[n]` must map to available rows). If citation support fails, it returns a guarded fallback. |
| `Synthesize Final` | Combines sub-answers into a single final response and enforces a final citation contract against collected citation rows. If citations are missing/invalid, it falls back to citation-valid content. |
